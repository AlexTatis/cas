...
const project = projects[+route.params.id - 1];
...
<template>
    <Stories v-if="storiesVisible || width >= 1280" :stories="project.images" class="transition-all xl:h-fit h-dvh w-dvw overflow-hidden xl:w-1/3 fixed xl:right-4 xl:top-1/2 xl:max-h-full xl:-translate-y-1/2 rounded-xl xl:py-4 drop-shadow-lg border-4 border-stone-900" :loop="true" :interval="5000"></Stories>
    ...
    <div class="w-full xl:w-2/3 p-2 sm:p-8">
        <div class="mb-8 bg-stone-700 px-6 py-4 rounded-xl top-8 drop-shadow-md border-4 border-stone-900 flex items-center justify-between flex-col gap-4 sm:gap-8 sm:flex-row">
            <div class="flex items-center gap-6">
                ...
                <h1 class="text-4xl md:text-6xl">{{ project.title }}</h1>
            ...
            <div class="flex gap-4 flex-wrap-reverse justify-center">
                <div class="badge badge-xs font-mono text-white lg:badge-lg" :class="{ 'badge-info': area == 'Creatividad', 'badge-success': area == 'Actividad', 'badge-error': area == 'Servicio' }" v-for="area in project.areas">{{ area }}</div>
            </div>
        </div>
        <Suspense>
            <Markdown :md="project.content" class="p-2 font-mono" />
